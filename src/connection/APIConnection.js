...
async function fetchCountryStats(apiKey, country, referencedDate) {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      console.error(
        "Connectivity Issue | Error fetching country stats:",
        error.message,
      );
...
async function fetchCountryComparisionStats(apiKey, countries, referencedDate) {
  ...
  try {
    ...
    if (error instanceof TypeError) {
      console.error(
        "Connectivity Issue | Error fetching comparision stats:",
        error.message,
      );
      throw new errors.networkError(
        "Network error. Please check your internet connection.",
  ...
  async function fetchSupportedCountries(apiKey) {
    const url = Endpoint.supportedCountries;
    ...
    try {
      ...
      if (error instanceof TypeError) {
        console.error(
          "Connectivity Issue | Error fetching supported countries:",
          error.message,
        );
        throw new errors.networkError(
          "Network error. Please check your internet connection.",
...
export const dataSource = {
  ...
  supportedCountries : fetchSupportedCountries
};
